...
class AFF_MCP_Connector {
	...
	private function tool_definitions() {
		...
		$tools = array(
			...
			array(
				'name'        => 'get_term_meta',
				'description' => 'Get all meta for a taxonomy term, or a single key. Includes WooCommerce category thumbnail_id and its resolved image URL when present.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => array(
						'term_id' => $int,
						'key'     => array( 'type' => 'string', 'description' => 'Optional single meta key; omit for all meta' ),
					),
			...
			array(
				...
				'description' => 'Set a single term meta value (e.g. Rank Math/Yoast term SEO fields, or arbitrary meta).',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => array(
						'term_id' => $int,
			...
			array(
				...
				'description' => 'Set the image (thumbnail) on a WooCommerce product category. Provide either media_id (existing attachment) or image_url (downloaded into the library first). Use media_id 0 to remove.',
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => array(
						'term_id'   => $int,
						'media_id'  => array( 'type' => 'integer', 'description' => 'Existing attachment ID, or 0 to remove' ),
						'image_url' => array( 'type' => 'string', 'description' => 'Public https image URL to sideload and assign' ),
						'alt'       => $str,
					),
					'required'   => array( 'term_id' ),
				),
	...
	private function run_tool( $id, $name, $args ) {
		try {
			switch ( $name ) {
				...
				case 'get_term_meta':   $data = $this->tool_get_term_meta( $args ); break;
				case 'update_term_meta': $data = $this->tool_update_term_meta( $args ); break;
				case 'set_category_image': $data = $this->tool_set_category_image( $args ); break;
	...
	private function tool_get_term_meta( $args ) {
		$term_id = (int) $args['term_id'];
		if ( ! get_term( $term_id ) ) throw new Exception( 'Term not found' );
		if ( ! empty( $args['key'] ) ) {
			return array( 'term_id' => $term_id, 'key' => $args['key'], 'value' => get_term_meta( $term_id, sanitize_key( $args['key'] ), true ) );
		}
		$all = get_term_meta( $term_id );
		$flat = array();
		foreach ( $all as $k => $v ) $flat[ $k ] = is_array( $v ) && 1 === count( $v ) ? $v[0] : $v;
		$thumb_id  = isset( $flat['thumbnail_id'] ) ? (int) $flat['thumbnail_id'] : 0;
		...
		return array(
			...
			'thumbnail_broken' => $thumb_id ? ( false === $thumb_url ) : null,
		);
	}

	private function tool_update_term_meta( $args ) {
		$term_id = (int) $args['term_id'];
		if ( ! get_term( $term_id ) ) throw new Exception( 'Term not found' );
		update_term_meta( $term_id, sanitize_key( $args['key'] ), sanitize_text_field( $args['value'] ) );
		return array( 'term_id' => $term_id, 'key' => sanitize_key( $args['key'] ), 'updated' => true );
	}

	private function tool_set_category_image( $args ) {
		$term_id = (int) $args['term_id'];
		if ( ! get_term( $term_id ) ) throw new Exception( 'Term not found' );
		if ( isset( $args['media_id'] ) && 0 === (int) $args['media_id'] && empty( $args['image_url'] ) ) {
			delete_term_meta( $term_id, 'thumbnail_id' );
			return array( 'term_id' => $term_id, 'thumbnail_id' => 0, 'removed' => true );
		}
		$media_id = isset( $args['media_id'] ) ? (int) $args['media_id'] : 0;
		if ( ! $media_id && ! empty( $args['image_url'] ) ) {
			$url = esc_url_raw( $args['image_url'] );
			if ( 0 !== strpos( $url, 'http' ) ) throw new Exception( 'A valid http(s) image URL is required.' );
			require_once ABSPATH . 'wp-admin/includes/media.php';
			require_once ABSPATH . 'wp-admin/includes/file.php';
			require_once ABSPATH . 'wp-admin/includes/image.php';
			$media_id = media_sideload_image( $url, 0, null, 'id' );
			if ( is_wp_error( $media_id ) ) throw new Exception( $media_id->get_error_message() );
			if ( ! empty( $args['alt'] ) ) update_post_meta( $media_id, '_wp_attachment_image_alt', sanitize_text_field( $args['alt'] ) );
		}
		if ( ! $media_id || ! wp_attachment_is_image( $media_id ) ) throw new Exception( 'Provide a valid media_id or image_url.' );
